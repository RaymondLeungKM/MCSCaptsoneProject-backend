...
def upgrade() -> None:
    ...
    indexes = {index['name'] for index in inspector.get_indexes('game_sessions')}
    if op.f('ix_game_sessions_id') not in indexes:
        op.create_index(
            op.f('ix_game_sessions_id'), 'game_sessions', ['id'], unique=False
        )
    if op.f('ix_game_sessions_child_id') not in indexes:
        op.create_index(
